...
class Temperature_Poll_Thread(threading.Thread):
    def __init__(self, time_sleep = 0.1, file_loc = "../Testdata/core_temp/", name = 'temperature_poll_thread'):
    ...
    def open_file(self):
        ...
        if not os.path.exists(file_loc_date):
            ...
            counter = self.read_file_identifer(file_loc_date)
    ...
    def poll_core_temp(self):
        ...
        curr_core_temp = float(curr_core_temp_str[4:-2]) # temp=44.4'C
        #time = strftime('%H:%M:%S')
